...
class User(Node):
	...
	def get_info(self, info):
		response = {}
		if ("interests" in info) or ("name" in info) or ("status" in info): 	
			if "interests" in info:
				query = " SELECT users.name, users.status, "\
					+   " json_agg(row_to_json((SELECT d FROM (SELECT users_interest.interest_id AS id, users_interest.properties) d))) AS  interests "\
					+   " FROM users "\
					+   " LEFT OUTER JOIN users_interest on (users.username = users_interest.username) WHERE users.username = %s  GROUP BY users.username; "
			else:
				query = " SELECT users.name, users.status  FROM users WHERE users.username = %s ; "
			variables = (self.username, )
			record = QueryHandler.get_results(query, variables) 

			for key in record[0].keys():
				if key in info:
					response[key] = record[0][key]


		if 'l_photo' in info:
			response['l_photo'] = base64.b64encode(Dp(self.username).get_dp_version('L'))
