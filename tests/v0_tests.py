...
class ProfilePicServiceTest(unittest.TestCase):
    ...
    def test_profile_pic_updation(self):
        ...
        S3(self._profile_pic_bucket).check_exists(self._large_version_name)
        S3(self._profile_pic_bucket).check_exists(self._small_version_name)

        s3_small_file_url = "https://%s.s3.amazonaws.com/%s" % (
            self._profile_pic_bucket, self._small_version_name)
        s3_file_response = requests.get(s3_small_file_url)
        self.assertEqual(200, s3_file_response.status_code)


        s3_large_file_url = "https://%s.s3.amazonaws.com/%s" % (
            self._profile_pic_bucket, self._large_version_name)
        s3_file_response = requests.get(s3_large_file_url)
        self.assertEqual(200, s3_file_response.status_code)
