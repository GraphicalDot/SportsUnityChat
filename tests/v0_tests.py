...
class ArticlePollAnswerSubmissionTest(unittest.TestCase):
	...
	def test_post_poll_response(self):
		payload = {"username": self._username, "password": self._password, "poll_answer": self._user_response, "article_id": self._article_id}
		payload.update(extra_params_dict)
		...
		assert results[0]['username'] == self._username
		assert results[0]['poll_answer'] == self._user_response
...
# class GroupAllocationTest(unittest.TestCase):
# 	def setUp(self):
# 		test_utils.delete_user(username = self._username)
# 		test_utils.create_user(username = self._username, password = self._password, phone_number = self._phone_number)
