...
do_route(ServerHost, Access, Plugins, Host, From, To, Packet) ->
    #xmlel{name = Name, attrs = Attrs ,children = Children} = Packet,
    case To of
	#jid{luser = <<>>, lresource = <<>>} ->
	    case Name of
		<<"iq">> ->
		...
		<<"message">> ->
		    case xml:get_subtag_cdata(Packet, <<"subject">>) of
		        <<"New Live matches">> ->
		            SubscribedMatch = xml:get_subtag_cdata(Packet, <<"body">>),
                    ejabberd_hooks:run(create_match_node, ServerHost, [ServerHost, Host, From, SubscribedMatch]);
		        _ -> ok
		    end,
