...
class NearbyUsersWithSameInterestsTests(unittest.TestCase):
    ...
    def test_get(self):
        ...
        self.data = {'username': 'test_4', 'apk_version': self.apk_version, 'udid': self.udid, 'nearby_users': str(self.all_nearby_users)}
        ...
        QueryHandler.execute(query, (self.epoch_time - 8000,))
        self.expected_result_dict = {"friends": {"test_5": ["test_interest_2", "test_interest_1"]},
    ...
    def test_no_nearby_users(self):
        self.all_nearby_users = []
        self.data = {'username': 'test_4', 'apk_version': self.apk_version, 'udid': self.udid, 'nearby_users': str(self.all_nearby_users)}
        response = requests.get(self.url, data=self.data)
        self.assert_response_status(response, "Bad Request: User has no users nearby!", settings.STATUS_400)

    def test_no_interests(self):
        for user in self.all_nearby_users:
            user['interests'] = ""
        self.data = {'username': 'test_4', 'apk_version': self.apk_version, 'udid': self.udid, 'nearby_users': str(self.all_nearby_users)}
        response = requests.get(self.url, data=self.data)
        self.assert_response_status(response, "Bad Request: This user has NO interests!", settings.STATUS_400)
